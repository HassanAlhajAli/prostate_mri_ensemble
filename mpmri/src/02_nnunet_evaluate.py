...
# --- MULTI-PARAMETRIC PATHS ---
ROOT = Path(__file__).resolve().parents[1]
RAW_DATA_DIR = ROOT / "data" / "nnUNet_data" / "nnUNet_raw" / "Dataset501_ProstateMPMRI"
MODEL_DIR = ROOT / "checkpoints" / "nnunet_mpmri"
PREDICTIONS_DIR = ROOT / "reports" / "predictions_mpmri"
REPORT_PATH = ROOT / "reports" / "phase3_mpmri_evaluation.json"


def ensure_model_files() -> None:
    """Ensures checkpoints are inside a fold_0 subfolder as required by nnU-Net."""
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    fold_dir = MODEL_DIR / "fold_0"
    fold_dir.mkdir(parents=True, exist_ok=True)

    for src_name in ["checkpoint_final.pth", "checkpoint_best.pth"]:
        root_path = MODEL_DIR / src_name
        fold_path = fold_dir / src_name
        # If the file is loose in the main dir, move it to fold_0
        if root_path.exists() and not fold_path.exists():
            shutil.move(str(root_path), str(fold_path))


def get_image_and_label_files() -> List[Tuple[List[Path], Path]]:
    """Groups T2, ADC, and DWI modalities together for each patient."""
    image_dir = RAW_DATA_DIR / "imagesTs"
    label_dir = RAW_DATA_DIR / "labelsTs"
    
    # Find all unique patients based on the T2 (_0000) file
    t2_files = sorted(image_dir.glob("*_0000.nii.gz"))
    pairs: List[Tuple[List[Path], Path]] = []
    
    for t2_path in t2_files:
        case_id = t2_path.name.replace("_0000.nii.gz", "")
        
        img_0 = image_dir / f"{case_id}_0000.nii.gz"
        img_1 = image_dir / f"{case_id}_0001.nii.gz"
        img_2 = image_dir / f"{case_id}_0002.nii.gz"
        label_path = label_dir / f"{case_id}.nii.gz"
        
        if img_0.exists() and img_1.exists() and img_2.exists() and label_path.exists():
            pairs.append(([img_0, img_1, img_2], label_path))
        else:
            print(f"Skipping {case_id}: Missing one or more modalities or labels.")
...
def run_inference(image_lists: List[List[Path]]) -> None:
    ...
    # Passes the list of modalities to the predictor
    predictor.predict_from_files(
        [[str(p) for p in img_list] for img_list in image_lists],
        str(PREDICTIONS_DIR),
...
def evaluate_predictions() -> dict:
    ...
    for img_list, label_path in pairs:
        # Extract case ID from the T2 file name
        case_id = img_list[0].name.replace("_0000.nii.gz", "")
...
def main() -> None:
    ...
    # Extract just the image lists to feed to the predictor
    image_lists = [img_list for img_list, _ in image_and_label_pairs]
    
    if not image_lists:
        raise RuntimeError("No test images found for evaluation")
        
    run_inference(image_lists)
