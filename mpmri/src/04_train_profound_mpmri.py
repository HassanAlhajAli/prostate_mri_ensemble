...
# --- PATHS (Encapsulated exactly in mpmri) ---
SCRIPT_DIR = Path(__file__).resolve().parent
MPMRI_DIR = SCRIPT_DIR.parent
ROOT_DIR = MPMRI_DIR.parent

CHECKPOINT_DIR = MPMRI_DIR / "checkpoints" / "profound_mpmri"

# CORRECTED: Pointing to MPMRI_DIR instead of ROOT_DIR
FEATURE_DIR = MPMRI_DIR / "data" / "03_frozen_features_mpmri"
TRAIN_FEATURE_DIR = FEATURE_DIR / "train"
TEST_FEATURE_DIR = FEATURE_DIR / "test"

# Point to the Dataset501 labels
TRAIN_LABEL_DIR = MPMRI_DIR / "data" / "nnUNet_data" / "nnUNet_raw" / "Dataset501_ProstateMPMRI" / "labelsTr"
TEST_LABEL_DIR = MPMRI_DIR / "data" / "nnUNet_data" / "nnUNet_raw" / "Dataset501_ProstateMPMRI" / "labelsTs"

REPORTS_DIR = MPMRI_DIR / "reports"
PREDICTIONS_DIR = REPORTS_DIR / "profound_predictions_mpmri"

TARGET_SHAPE = (64, 160, 160)
OPTIMAL_THRESHOLD = 0.65  # The champion threshold!

def set_seed(seed: int = 42):
    """Strict reproducibility to guarantee stable performance"""
    random.seed(seed)
    np.random.seed(seed)
...
def main():
    ...
    print(f"[mpMRI] Device: {device} | Strict Determinism: ON | Target Threshold: {OPTIMAL_THRESHOLD}")
    ...
    best_val_dice = -1.0
    best_model_path = CHECKPOINT_DIR / "final_mpmri_champion.pt"
    epochs = 30 

    print("\n--- PHASE 1: TRAINING MPMRI CHAMPION MODEL ---")
    ...
    report = {
        "model": "AdvancedProFoundDecoder3D (mpMRI Champion)",
    ...
    with open(REPORTS_DIR / "final_profound_mpmri_test_metrics.json", "w") as f:
        json.dump(report, f, indent=4)
        
    print(f"\n🏆 FINAL MPMRI TEST DICE: {mean_test_dice:.4f}")
    print(f"✅ Predictions saved to: {PREDICTIONS_DIR}")
    print(f"✅ Report saved to: {REPORTS_DIR / 'final_profound_mpmri_test_metrics.json'}")
