...
OUTPUT_DIR = WORKSPACE_DIR / "data" / "05_sammed3d_unprompted_native_predictions"
...
def process_patient(patient_id: str, output_root: Path, threshold: float) -> Dict[str, object]:
    ...
    if not logits_path.exists():
        raise FileNotFoundError(f"Missing SAM-Med3D logits: {logits_path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Standalone SAM-Med3D unprompted Dice evaluation on the fixed test split.")
    ...
    save_json(REPORT_DIR / "sammed3d_unprompted_eval.json", report)
    save_json(output_root / "sammed3d_unprompted_summary.json", summary)

    print(f"SAM-Med3D unprompted evaluation complete for {len(results)} patients.")
