...
OUTPUT_DIR = WORKSPACE_DIR / "data" / "05_sammed3d_prompt_native_predictions"
...
def process_patient(
    ...
    method_out_dir = output_root / "sammed3d"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prompt-driven SAM-Med3D evaluation on the fixed test split.")
    ...
    if not CHECKPOINT_PATH.exists():
        raise FileNotFoundError(f"Missing SAM-Med3D checkpoint: {CHECKPOINT_PATH}")
    ...
    save_json(REPORT_DIR / "sammed3d_prompt_eval.json", report)
    save_json(output_root / "sammed3d_prompt_summary.json", summary)

    print(f"SAM-Med3D prompt evaluation complete for {len(results)} patients.")
