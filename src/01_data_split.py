...
NNUNET_DATASET_DIR = WORKSPACE_DIR / "data" / "nnUNet_data" / "nnUNet_raw" / "Dataset500_PROMIS"
...
def copy_patient_files(patient_list: List[str], is_train: bool, raw_dir: Path, target_dir: Path) -> None:
    ...
    for pid in tqdm(patient_list, desc=f"Processing {phase_name}", unit="scan"):
        src_img = raw_dir / pid / "t2.nii.gz"
        src_lbl = raw_dir / pid / "lesion_ordered.nii.gz"
        
        dst_img = target_dir / img_folder / f"{pid}_0000.nii.gz"
    ...
    if missing_data_count > 0:
        logging.warning(f"Skipped {missing_data_count} patients due to missing t2 or lesion files.")
...
def main() -> None:
    logging.info("Starting data split pipeline with mask binarization...")
