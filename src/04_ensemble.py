...
PROFOUND_FEATURE_DIR = WORKSPACE_DIR / "data" / "02_frozen_features"
PROFOUND_TRAIN_FEATURE_DIR = PROFOUND_FEATURE_DIR / "train"
PROFOUND_TEST_FEATURE_DIR = PROFOUND_FEATURE_DIR / "test"
PROFOUND_CHECKPOINT_CANDIDATES = [
    WORKSPACE_DIR / "checkpoints" / "profound_baseline" / "profound_baseline.pt",
    WORKSPACE_DIR / "checkpoints" / "profound_baseline.pt",
    WORKSPACE_DIR / "checkpoints" / "profound_baseline" / "final_champion.pt",
    WORKSPACE_DIR / "checkpoints" / "phase3" / "best.pt",
]

REPORT_DIR = WORKSPACE_DIR / "reports"
ENSEMBLE_PRED_DIR = REPORT_DIR / "ensemble_predictions"
ENSEMBLE_CACHE_DIR = REPORT_DIR / "ensemble_cache"
METRICS_PATH = REPORT_DIR / "ensemble_metrics.json"

PHASE4_CHECKPOINT_DIR = WORKSPACE_DIR / "checkpoints" / "phase4"
...
def list_case_ids(image_dir: Path) -> List[str]:
    return sorted(case_id_from_image_path(path) for path in image_dir.glob("*.nii.gz"))
...
class NnUNetProbabilitySource:
    ...
    def generate_split(self, image_dir: Path, split_dir: Path, force: bool = False) -> None:
        ...
        expected = [split_dir / f"{case_id_from_image_path(path)}_prob.npy" for path in image_files]
...
def load_profound_model(device: torch.device) -> nn.Module:
    ...
    for candidate in PROFOUND_CHECKPOINT_CANDIDATES:
        if candidate.exists():
            checkpoint_path = candidate
            break
    if checkpoint_path is None:
        raise FileNotFoundError(f"Missing ProFound checkpoint. Tried: {PROFOUND_CHECKPOINT_CANDIDATES}")
...
def copy_standalone_baselines(case_ids: Sequence[str]) -> None:
    baseline_root = ENSEMBLE_PRED_DIR / "baselines"
    nnunet_dst = baseline_root / "nnunet" / "test"
    profound_dst = baseline_root / "profound" / "test"
    nnunet_dst.mkdir(parents=True, exist_ok=True)
    profound_dst.mkdir(parents=True, exist_ok=True)
    for case_id in case_ids:
        src = WORKSPACE_DIR / "data" / "nnUNet_data" / "nnUNet_results" / "predictions_test_final" / f"{case_id}.nii.gz"
        if src.exists():
            shutil.copy2(src, nnunet_dst / src.name)
        src = WORKSPACE_DIR / "reports" / "profound_predictions" / f"{case_id}_pred.nii.gz"
        if src.exists():
            shutil.copy2(src, profound_dst / f"{case_id}.nii.gz")
...
def generate_profound_split(split: str, case_ids: Sequence[str], model: nn.Module, device: torch.device, force: bool = False) -> None:
    ...
    for case_id in tqdm(case_ids, desc=f"profound-{split}"):
        ...
        if not force and prob_path.exists() and mask_path.exists():
            continue
        feature_path = (PROFOUND_TRAIN_FEATURE_DIR if split == "train" else PROFOUND_TEST_FEATURE_DIR) / f"{case_id}_profound.npy"
        image_path = profound_image_path(split, case_id)
        if not feature_path.exists() or not image_path.exists():
            raise FileNotFoundError(f"Missing ProFound input for {case_id} in {split}")
        feature = np.load(feature_path).astype(np.float32)
        ...
        write_mask((probs >= 0.5).astype(np.uint8), reference, mask_path)
...
def prepare_caches(args: argparse.Namespace, device: torch.device) -> None:
    ...
    copy_standalone_baselines(test_ids)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase 4 ensemble fusion and comparison.")
